...
class PhilipsPhysioLog:
    ...
    def align(self, trigger_method='gradient_log', which_grad='y'):
        ...
        if trigger_method == 'gradient_log':
            ...
            raise ValueError("Choose from 'gradient_log', 'interpolate', or 'vol_triggers'")
    ...
    def _determine_triggers_by_interpolation(self):
        ...
        OFFSET = 30 #int((166./500) * self.sf)
        assumed_end = self.c_end_idx - OFFSET
        ...
        diffs = np.array([int(np.floor(self.trs)) if i % p == 0
                          else int(np.ceil(self.trs)) for i in range(self.n_trig)])
        #diffs[0] += assumed_start
        #self.real_triggers = np.cumsum(diffs)
        self.real_triggers = assumed_end - np.cumsum(diffs)[::-1]
...
if __name__ == '__main__':
    ...
    def _run_parallel(log):
        ...
        try:
            l = PhilipsPhysioLog(f=log, tr=tr, n_dyns=vols, sf=496, manually_stopped=False)
    ...
    wrong = jl.Parallel(n_jobs=1)(jl.delayed(_run_parallel)(log) for log in logs)
